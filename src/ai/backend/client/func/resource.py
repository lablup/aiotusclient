...
class Resource(BaseFunction):
    ...
    async def list(cls):
        ...
        """
        rqst = Request(api_session.get(), 'GET', '/resource/presets')
    ...
    async def check_presets(cls):
        ...
        information.
        """
        rqst = Request(api_session.get(), 'POST', '/resource/check-presets')
        async with rqst.fetch() as resp:
            return await resp.json()
    ...
    async def get_docker_registries(cls):
        ...
        """
        rqst = Request(api_session.get(), 'GET', '/config/docker-registries')
        async with rqst.fetch() as resp:
            return await resp.json()
    ...
    async def usage_per_month(cls, month: str, group_ids: Sequence[str]):
        ...
        """
        rqst = Request(api_session.get(), 'GET', '/resource/usage/month')
        rqst.set_json({
            'month': month,
            'group_ids': group_ids,
        })
    ...
    async def usage_per_period(cls, group_id: str, start_date: str, end_date: str):
        ...
        """
        rqst = Request(api_session.get(), 'GET', '/resource/usage/period')
        rqst.set_json({
            'group_id': group_id,
    ...
    async def get_resource_slots(cls):
        ...
        """
        rqst = Request(api_session.get(), 'GET', '/config/resource-slots')
        async with rqst.fetch() as resp:
            return await resp.json()
    ...
    async def get_vfolder_types(cls):
        rqst = Request(api_session.get(), 'GET', '/config/vfolder-types')
        async with rqst.fetch() as resp:
            return await resp.json()
    ...
    async def recalculate_usage(cls):
        rqst = Request(api_session.get(), 'POST', '/resource/recalculate-usage')
        async with rqst.fetch() as resp:
            return await resp.json()
    ...
    async def user_monthly_stats(cls):
        rqst = Request(api_session.get(), 'GET', '/resource/stats/user/month')
        async with rqst.fetch() as resp:
            return await resp.json()
    ...
    async def admin_monthly_stats(cls):
        rqst = Request(api_session.get(), 'GET', '/resource/stats/admin/month')
        async with rqst.fetch() as resp:
            return await resp.json()
